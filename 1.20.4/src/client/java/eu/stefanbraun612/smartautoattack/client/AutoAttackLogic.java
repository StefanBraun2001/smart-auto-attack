...
public class AutoAttackLogic {
	...
	public static void tick(MinecraftClient client) {
		...
		if (config.attackCadenceMode == SmartAutoAttackConfig.AttackCadenceMode.DEFAULT) {
			...
			if (player.getAttackCooldownProgress(0f) < 1.0f) {
				return;
			}
			...
			if (config.alwaysFullyCharge && player.getAttackCooldownProgress(0f) < 1.0f) {
	...
	private static boolean dimensionHasDayNightCycle(World world) {
		return world.getRegistryKey() != World.NETHER && world.getRegistryKey() != World.END;
	}

	private static void performAttack(MinecraftClient client, PlayerEntity player, SmartAutoAttackConfig config, Entity target) {
		if (target != null) {
	...
	private static void stop(MinecraftClient client, SmartAutoAttackConfig config, String message) {
	...
	private static void playAutoStopSound(MinecraftClient client, SmartAutoAttackConfig config) {
	...
	private static Entity resolveTarget(MinecraftClient client, SmartAutoAttackConfig config) {
	...
	private static boolean checkDurability(PlayerEntity player, SmartAutoAttackConfig config) {
	...
	private static boolean passesHungerSafety(PlayerEntity player, SmartAutoAttackConfig config) {
